  ...
  if (doPartnerFill) {
    try {
      ...
      if (live.length) {
        ...
        // Exclusividad del fijado: un partner fijado a OTRO espacio no debe salir aquí
        // aunque este espacio esté en "rotar todos". Así Axi (fijado en la barra
        // lateral) nunca aparece en los demás huecos que rotan.
        const pinnedElsewhere = new Set(
          Object.entries(cfg.partnerSlotPin || {})
            .filter(([k, v]) => v && k !== slotKey)
            .map(([, v]) => String(v)),
        );
        const pool = pinnedElsewhere.size ? live.filter((p: any) => !pinnedElsewhere.has(String(p.id))) : live;
        // Si todos los partners quedaron fijados a otros espacios, este hueco no rota
        // partners: cae a programmatic/house más abajo (nunca se muestra un fijado ajeno).
        if (pool.length) {
          ...
          const p = pool[(h + bucket + pos) % pool.length];
          return asPartner(p);
        }
      }
    } catch {}
