...
// Comisión de Onyx por copia (%). Fallback si no hay config guardada.
export const COPY_FEE_PCT = Number(process.env.ONYX_COPY_FEE_PCT || 30);
// Comisión efectiva (editable desde Admin → Onyx Copy). El resto va al trader.
export async function copyFeePct(): Promise<number> {
  try { const c: any = await copyConfig(); const v = Number(c?.feePct); return (isFinite(v) && v >= 0 && v <= 95) ? v : COPY_FEE_PCT; } catch { return COPY_FEE_PCT; }
}
