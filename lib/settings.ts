...
// ===== Direcciones de correo por función (evita que todo caiga en "support") =====
// Un solo lugar decide desde dónde SALEN los correos y a QUÉ buzón entra cada tipo
// de aviso. Todo puede reenviar al mismo Gmail, pero cada dirección permite filtrar.
export type MailRoutes = {
  from_name: string;   // nombre visible del remitente
  from_addr: string;   // dirección de envío (debe estar en el dominio verificado en Resend)
  support: string;     // soporte y contacto público
  billing: string;     // disputas, chargebacks y temas de pago
  botlab: string;      // leads de Onyx Bot Lab
  alerts: string;      // seguridad, self-test, backups (avisos internos)
};
export async function mailRoutes(): Promise<MailRoutes> {
  ...
  const domain = addr.includes('@') ? addr.split('@')[1] : 'onyxtradinglive.com';
  const firstAdmin = (process.env.ADMIN_EMAILS || '').split(',').map((s) => s.trim()).filter(Boolean)[0] || '';
  const def: MailRoutes = {
    from_name: name || 'Onyx Trading Live',
    from_addr: addr || `no-reply@${domain}`,
    support: (process.env.SUPPORT_EMAIL || `support@${domain}`).trim(),
    billing: (process.env.DISPUTE_ALERT_EMAIL || `billing@${domain}`).trim(),
    botlab: `botlab@${domain}`,
    alerts: (firstAdmin || `alerts@${domain}`).trim(),
  };
...
export const BOT_PLAN_MATRIX_DEF: BotPlanMatrix = {
  ...
  caps: {
    robots: { free: '1', trader: '3', black: '∞' },     // robots = cuentas del plan
...
export type BlogKeywords = {
  ...
  useGsc: boolean;           // alimentar la IA con las oportunidades reales de Search Console
...
export type BlogAutopilot = {
  ...
  autoEmail: boolean;      // al publicarse un artículo del piloto, enviarlo por email a la base
  emailSegment: string;    // segmento destino del auto-email (all, free, paid, connected…)
};
