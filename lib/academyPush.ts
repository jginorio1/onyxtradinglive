...
export async function pushUser(userId: string, title: string, body: string, url: string) {
  try { await sendPush(userId, { title, body, url }); } catch { /* nunca romper */ }
}

// Notifica a TODOS los alumnos activos de una academia (opcional: excluir a uno).
export async function pushStudents(mentorId: string, title: string, body: string, url: string, excludeId?: string) {
  try {
    const { data: enr } = await supabaseAdmin.from('academy_enrollments').select('student_id').eq('mentor_id', mentorId).eq('status', 'active');
    const ids = (enr || []).map((e: any) => e.student_id).filter((id: string) => id !== excludeId);
...
export async function pushAnnouncement(mentorId: string, body: string) {
  ...
  await pushStudents(mentorId, `📣 ${name}`, (body || '').slice(0, 120) || 'Nuevo anuncio', academyUrl(mentorId));
}
...
export async function pushDm(mentorId: string, fromId: string, toId: string, body: string) {
  ...
  await pushUser(toId, `💬 ${from}`, (body || '').slice(0, 120) || 'Te envió un mensaje', academyUrl(mentorId, '&open=chat'));
}
...
export async function pushWinApproved(mentorId: string, studentId: string) {
  const name = await acadName(mentorId);
  await pushUser(studentId, '🏆 ¡Logro publicado!', `Tu logro ya está en el muro de ${name}.`, academyUrl(mentorId, '&open=logros'));
}
export async function pushWinPending(mentorId: string, studentId: string) {
  const who = await nameOf(studentId);
  await pushUser(mentorId, '🏆 Logro por aprobar', `${who} subió un logro. Revísalo.`, academyUrl(mentorId, '&open=logros'));
}
// Clase en vivo próxima → a todos los alumnos.
export async function pushClassSoon(mentorId: string, title: string, joinUrl?: string) {
  await pushStudents(mentorId, '🔴 Clase en vivo pronto', `"${title}" empieza pronto. ¡Entra!`, joinUrl || academyUrl(mentorId, '&open=calendar'));
}
