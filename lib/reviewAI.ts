...
export async function draftReview(lang: 'es' | 'en', stars: number = (Math.random() < 0.8 ? 5 : 4), profile?: Profile, date?: string, used?: Set<string>): Promise<ReviewDraft | null> {
  ...
  const toneEs = st >= 5
    ...
    : 'Tono positivo pero con UN pequeño matiz o crítica menor (ej. me costó un poco al inicio, algo que mejorar), sin ser negativa.';
  const toneEn = st >= 5
    ? 'Very positive, enthusiastic tone, recommends without reservations.'
    : 'Positive but with ONE small caveat or minor critique (e.g. took me a bit to get the hang of it, something to improve), not negative.';
  const system = lang === 'es'
    ? `Eres un generador de reseñas realistas para una herramienta que arma robots de trading, sin saber de programación, para cuentas de fondeo (prop firms), compatible con MT4, MT5 y cTrader. El robot lleva dentro las reglas de riesgo, filtro de noticias y de sesión, y registra las operaciones en un panel. Escribe UNA reseña creíble, en primera persona, con la voz de un trader común y corriente (NO de un programador ni de alguien técnico). Habla como habla la gente normal: sencillo, cercano, natural, con alguna expresión coloquial. PROHIBIDO usar jerga de programador: nunca escribas \"código\", \"códigos\", \"sin código\", \"programar líneas\", \"script\", \"compilar\" ni \"variables\". Si quieres decir que es fácil, dilo con palabras de usuario común: \"sin saber de programación\", \"sin ser experto\", \"solo elegí mis reglas\", \"en unos minutos\", \"sin complicarme la vida\". Puedes incluir un detalle concreto y humano (mi prop firm, mi horario, que ahora duermo tranquilo, que dejé de saltarme mis reglas). SIN emojis, SIN promesas de ganancias garantizadas, que NO suene a anuncio. IMPORTANTE: BREVE y COMPLETA — 1 o 2 frases, máximo 30 palabras, que termine bien (no la dejes a medias). Variada (no empieces siempre igual). ${toneEs} Devuelve SOLO un JSON válido: {"text":"la reseña"}. Nada más.`
    : `You generate realistic reviews for a tool that builds trading bots, with no programming needed, for funded accounts (prop firms), for MT4, MT5 and cTrader. The bot carries risk rules, news and session filters inside, and logs trades to a dashboard. Write ONE believable first-person review in the voice of an everyday trader (NOT a programmer or a techie). Talk like a normal person: simple, warm, natural, a little casual. NEVER use programmer jargon: never write \"code\", \"coding\", \"no-code\", \"script\", \"compile\" or \"variables\". If you want to say it is easy, say it like a real user would: \"without knowing any programming\", \"I am not techy at all\", \"I just picked my rules\", \"in a few minutes\", \"no headaches\". You can include one concrete, human detail (my prop firm, my sessions, sleeping better now, finally sticking to my rules). NO emojis, NO guaranteed-profit claims, and it must NOT sound like an ad. IMPORTANT: SHORT and COMPLETE — 1 or 2 sentences, max 30 words, ending properly (do not leave it unfinished). Varied (do not always start the same way). ${toneEn} Return ONLY valid JSON: {"text":"the review"}. Nothing else.`;
  const user = lang === 'es'
    ? `Reseña de ${st} estrellas para un usuario cuyo contexto/resultado es: "${firm}". Máximo 30 palabras. Que no empiece con "Llevo". Recuerda: voz de usuario común, nada de jerga de programador (nada de "código").`
    : `A ${st}-star review for a user whose context/result is: "${firm}". Max 30 words. Remember: everyday-user voice, no programmer jargon (no "code").`;
