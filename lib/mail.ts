...
// Remitente por defecto tomado de la config de Admin (Direcciones de correo),
// con caché de 60s para no consultar la BD en cada envío. Si algo falla, cae al env.
let _fromCache: { at: number; from: string } | null = null;
async function resolvedFrom(): Promise<string> {
  if (_fromCache && Date.now() - _fromCache.at < 60000) return _fromCache.from;
  let from = process.env.SUPPORT_FROM_EMAIL || 'Onyx Trading Live <no-reply@onyxtradinglive.com>';
  try { const r = await mailRoutes(); if (r?.from_addr) from = fromLine(r); } catch {}
  _fromCache = { at: Date.now(), from };
  return from;
}
