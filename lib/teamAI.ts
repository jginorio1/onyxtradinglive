...
export async function onyxTeamAnswer(opts: { question: string; lang: Lang }): Promise<string> {
  ...
  const byCat: Record<string, number> = {};
  let leads = 0, unassigned = 0;
  const waiting: any[] = [];   // esperan respuesta del equipo
  for (const t of rows as any[]) {
    ...
    if (lastSender[t.id] === 'user' && t.status !== 'resolved') {
      const hrs = Math.round((Date.now() - new Date(lastAt[t.id] || t.updated_at).getTime()) / H);
      waiting.push({ subject: t.subject, email: t.email, hrs, priority: t.priority });
    }
  }
  ...
  let ctx = `RESUMEN (${rows.length} tickets recientes):\n`
    ...
    + `- Esperan respuesta del equipo: ${waiting.length}${waiting.length ? ` (más viejo: ${waiting[0].hrs}h)` : ''}\n`;
  if (waiting.length) {
    ctx += `\nPENDIENTES (esperan respuesta, del más viejo):\n`
      + waiting.slice(0, 12).map((w) => `- "${w.subject}" · ${w.email || '—'} · ${w.hrs}h${w.priority === 'high' ? ' · ALTA' : ''}`).join('\n') + '\n';
  }

  // 3) ¿Se menciona un cliente por correo? → historial de ese cliente
  const email = (question.match(/[\w.+-]+@[\w-]+\.[\w.-]+/) || [])[0]?.toLowerCase();
  if (email) {
    const { data: cli } = await supabaseAdmin.from('support_tickets')
      .select('id,subject,category,status,created_at,updated_at').ilike('email', email).order('created_at', { ascending: false }).limit(15);
    let prof: any = null;
    try { const { data } = await supabaseAdmin.from('profiles').select('email,full_name,plan,created_at').ilike('email', email).maybeSingle(); prof = data; } catch {}
    ctx += `\nCLIENTE ${email}:\n`;
    if (prof) ctx += `- ${nameOf(prof)}${prof.plan ? ` · plan ${String(prof.plan).toUpperCase()}` : ''}${prof.created_at ? ` · desde ${new Date(prof.created_at).toLocaleDateString()}` : ''}\n`;
