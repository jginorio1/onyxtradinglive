...
export async function proposalPdf(d: ProposalData, opts?: { company?: string; lang?: 'es' | 'en' }): Promise<Uint8Array> {
  // @ts-ignore
  ...
  let y = 752;
  T(d.candidateName
    ? (en ? `Prepared for: ${d.candidateName}  (role: ${d.levelName})` : `Preparado para: ${d.candidateName}  (plaza: ${d.levelName})`)
    : (en ? `Join us as ${d.levelName}` : `Unete como ${d.levelName}`), 40, y, 10.5, font, gray);
  y -= 22;
  T(en ? 'Sell once. Get paid every month.' : 'Vende una vez. Cobra cada mes.', 40, y, 20, bold, dark);
  y -= 20;
  const headScenario = d.hasTeam
    ? (en ? `Your own sales + a team of ${d.teamSize}: in 12 months you reach ${money(potential)}/mo.` : `Con tu venta directa + un equipo de ${d.teamSize}, en 12 meses llegas a ${money(potential)}/mes.`)
    : (en ? `With ${d.clientsPerMonth} new clients per month, in 12 months you reach ${money(potential)}/mo recurring.` : `Con ${d.clientsPerMonth} clientes nuevos al mes, en 12 meses llegas a ${money(potential)}/mes recurrente.`);
  ...
  gcell(40, 165, en ? 'YOUR 1ST MONTH PER CLIENT' : 'TU 1.ER MES POR CLIENTE', money(d.perFirst), en ? `${d.firstDirectPct}% of the first payment` : `${d.firstDirectPct}% del primer pago`, gold);
  gcell(215, 165, en ? 'EVERY MONTH AFTER' : 'CADA MES DESPUES', money(d.perMonthly), en ? `${d.residualDirectPct}% while active` : `${d.residualDirectPct}% mientras siga activo`, green);
  gcell(390, 165, en ? 'THAT CLIENT IN 1 YEAR' : 'ESE CLIENTE EN 1 ANO', money(d.perYear1), en ? 'from a single client' : 'de un solo cliente', dark);
  y -= 66;
  ...
  T(en ? 'Where your money comes from' : 'De donde viene tu dinero', 52, chartTop - 16, 10.5, bold, dark);
  T(clean(en ? `Year 1${d.hasTeam ? ' - direct + team' : ` - ${d.activeYear1} clients`}` : `Ano 1${d.hasTeam ? ' - directo + equipo' : ` - ${d.activeYear1} clientes`}`), 52, chartTop - 30, 8, font, gray);
  ...
  leg(green, `${en ? 'Recurring' : 'Recurrente'} ${Math.round(d.mix3.residualDirect / total3 * 100)}%`);
  leg(gold, `${en ? '1st month' : '1.er mes'} ${Math.round(d.mix3.firstDirect / total3 * 100)}%`);
  if (d.hasTeam) leg(purple, `${en ? 'Team' : 'Equipo'} ${Math.round(d.mix3.team / total3 * 100)}%`);
  T(money(d.hasTeam ? d.combinedTotalYear1 : d.totalYear1), 160, ly - 4, 12, bold, dark);
  T(en ? 'year 1 total' : 'total ano 1', 160, ly - 16, 7.5, font, gray);
  ...
  T(en ? 'The snowball effect' : 'El efecto bola de nieve', bx + 12, chartTop - 16, 10.5, bold, dark);
  T(en ? (d.hasTeam ? 'Direct + team, month 1 to 12' : 'Monthly income, month 1 to 12') : (d.hasTeam ? 'Directo + equipo, mes 1 a 12' : 'Ingreso mensual, mes 1 a 12'), bx + 12, chartTop - 30, 8, font, gray);
  ...
  const lbl = `${money(series[series.length - 1])}/${en ? 'mo' : 'mes'}`;
  T(lbl, lastX - bold.widthOfTextAtSize(lbl, 10), lastY + 7, 10, bold, blue);
  T('M1', gx0, gy0 - 11, 7, font, gray); T(en ? 'month 12' : 'mes 12', gx1 - 26, gy0 - 11, 7, font, gray);
  ...
  if (d.hasTeam) {
    ...
    T(en ? 'What you earn from your team' : 'Lo que ganas de tu equipo', 52, y - 4, 11, bold, purple);
    const teamLine = d.level === 'l2'
      ? (en ? `As ${d.levelName}: +${d.overrides.l1Res}% of your direct team (${d.teamSize}) and +${d.overrides.l2Res}% of the network (${d.networkSize}).` : `Como ${d.levelName}: +${d.overrides.l1Res}% de tu equipo directo (${d.teamSize}) y +${d.overrides.l2Res}% de la red (${d.networkSize}).`)
      : (en ? `As ${d.levelName}: +${d.overrides.l1Res}% of everything your team sells (${d.teamSize} reps).` : `Como ${d.levelName}: +${d.overrides.l1Res}% de todo lo que venda tu equipo (${d.teamSize} vendedores).`);
    T(clean(teamLine), 52, y - 20, 9.5, font, ink);
    T(clean(en
      ? `That adds ${money(d.overridePotentialMonthly)}/mo by month 12  ->  ${money(d.overrideTotalYear1)} extra in year 1, without you selling those clients.`
      : `Eso suma ${money(d.overridePotentialMonthly)}/mes al mes 12  ->  ${money(d.overrideTotalYear1)} extra en el ano 1, sin vender tu esos clientes.`), 52, y - 33, 9.5, font, gray);
    y -= 58;
  }

  // ---- Tabla por clientes (tus ventas directas) ----
  T(en ? 'Your direct sales, if you close more clients' : 'Tus ventas directas, si cierras mas clientes', 40, y, 12, bold, dark); y -= 18;
  const cols = [40, 210, 330, 450];
  T(en ? 'Clients' : 'Clientes', cols[0], y, 9, bold, gray); T(en ? '1st month' : '1.er mes', cols[1], y, 9, bold, gold); T(en ? 'Each month' : 'Cada mes', cols[2], y, 9, bold, green); T(en ? 'Year 1' : 'Ano 1', cols[3], y, 9, bold, dark);
  y -= 4; page.drawLine({ start: { x: 40, y }, end: { x: 555, y }, thickness: 0.7, color: soft }); y -= 14;
  for (const rr of d.rows) {
    T(`${rr.clients} ${en ? 'clients' : 'clientes'}`, cols[0], y, 11, font, ink);
  ...
  // ---- Por qué Onyx ----
  T(en ? 'Why Onyx is your best option' : 'Por que Onyx es tu mejor opcion', 40, y, 12, bold, dark); y -= 18;
  const why = (t: string, s2: string) => { T(clean(`- ${t}: `), 40, y, 10, bold, ink); T(clean(s2), 40 + bold.widthOfTextAtSize(`- ${t}: `, 10), y, 10, font, gray); y -= 15; };
  why(en ? 'Income that repeats' : 'Ingreso que se repite', en ? 'you never start from zero: you get paid every month the client stays.' : 'no arrancas de cero: cobras cada mes que el cliente sigue.');
  if (d.hasTeam) why(en ? 'You earn while your team sells' : 'Ganas mientras tu equipo vende', en ? 'your override comes in even when you do not close that sale.' : 'tu override entra aunque tu no cierres esa venta.');
  else why(en ? 'You move up on your own' : 'Subes de nivel solo', `${d.ladder.advisor} -> ${d.ladder.lead} -> ${d.ladder.director}${en ? ', and you earn from your team.' : ', y ganas de tu equipo.'}`);
  why(en ? 'Kick-off bonuses' : 'Bonos de arranque', en ? 'your first sales and monthly goals add extra.' : 'tus primeras ventas y las metas del mes suman extra.');
  y -= 8;

  // ---- Carrera / overrides (según el nivel de entrada) ----
  if (d.includeOverrides) {
    T(en ? 'Your career' : 'Tu carrera', 40, y, 12, bold, dark); y -= 18;
    if (d.level === 'vendedor') {
      T(clean(en ? `${d.ladder.advisor}  ->  ${d.ladder.lead}: when you reach ${d.ladder.toLead} active clients.` : `${d.ladder.advisor}  ->  ${d.ladder.lead}: al llegar a ${d.ladder.toLead} clientes activos.`), 40, y, 10.5, font, gray); y -= 15;
      T(clean(en ? `${d.ladder.lead}  ->  ${d.ladder.director}: with ${d.ladder.toDirector} on their team.` : `${d.ladder.lead}  ->  ${d.ladder.director}: con ${d.ladder.toDirector} en su equipo.`), 40, y, 10.5, font, gray); y -= 15;
      T(clean(en ? `As ${d.ladder.lead} you earn +${d.overrides.l1Res}% of your team; as ${d.ladder.director}, +${d.overrides.l2Res}%.` : `Como ${d.ladder.lead} ganas +${d.overrides.l1Res}% de tu equipo; como ${d.ladder.director}, +${d.overrides.l2Res}%.`), 40, y, 10.5, font, gray); y -= 20;
    } else if (d.level === 'l1') {
      T(clean(en ? `You start straight as ${d.ladder.lead}: you earn from your team from day one.` : `Entras directo como ${d.ladder.lead}: ganas de tu equipo desde el primer dia.`), 40, y, 10.5, font, gray); y -= 15;
      T(clean(en ? `Next step: ${d.ladder.director} (with ${d.ladder.toDirector} on your team) = +${d.overrides.l2Res}% of the network.` : `Siguiente paso: ${d.ladder.director} (con ${d.ladder.toDirector} en tu equipo) = +${d.overrides.l2Res}% de la red.`), 40, y, 10.5, font, gray); y -= 20;
    } else {
      T(clean(en ? `You start at the top as ${d.ladder.director}: the whole network below you.` : `Entras en lo mas alto como ${d.ladder.director}: toda la red debajo de ti.`), 40, y, 10.5, font, gray); y -= 15;
      T(clean(en ? `You earn +${d.overrides.l1Res}% of your direct team and +${d.overrides.l2Res}% of every level below.` : `Ganas +${d.overrides.l1Res}% de tu equipo directo y +${d.overrides.l2Res}% de cada nivel debajo.`), 40, y, 10.5, font, gray); y -= 20;
    }
  }

  // ---- Metas y bonos ----
  if (d.goal.clients > 0 || d.goal.amount > 0 || d.firstBonus) {
    T(en ? 'Goals and bonuses' : 'Metas y bonos', 40, y, 12, bold, dark); y -= 18;
    if (d.goal.bonus > 0 && (d.goal.clients > 0 || d.goal.amount > 0)) {
      const target = d.goal.clients > 0 ? `${d.goal.clients} ${en ? 'clients' : 'clientes'}` : `${money(d.goal.amount)} ${en ? 'in commission' : 'en comision'}`;
      T(clean(en ? `Monthly goal: ${target}  ->  ${money(d.goal.bonus)} bonus.` : `Meta del mes: ${target}  ->  bono de ${money(d.goal.bonus)}.`), 40, y, 10.5, font, gray); y -= 15;
    }
    if (d.firstBonus) { T(clean(en ? `Kick-off bonus: your first ${d.firstBonus.count} sales = ${money(d.firstBonus.bonus)} extra.` : `Bono de arranque: tus primeras ${d.firstBonus.count} ventas = ${money(d.firstBonus.bonus)} extra.`), 40, y, 10.5, font, gray); y -= 15; }
    y -= 6;
  }

  // ---- CTA + disclaimer ----
  page.drawRectangle({ x: 40, y: y - 34, width: 515, height: 40, color: dark });
  T(en ? 'Ready to start? Apply at onyxtradinglive.com/unete-ventas' : 'Listo para empezar? Postulate en onyxtradinglive.com/unete-ventas', 56, y - 20, 11, bold, rgb(1, 1, 1));
  y -= 54;
  T(en ? 'Examples calculated with current parameters. Commissions depend on the client continuing to pay.' : 'Ejemplos calculados con los parametros actuales. Las comisiones dependen de que el cliente siga pagando.', 40, y, 7.5, font, gray);
