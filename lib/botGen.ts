...
// MT4 solo maneja períodos estándar: mapeamos cada TF al más cercano soportado.
const TF4: Record<string, string> = {
  M1: 'PERIOD_M1', M2: 'PERIOD_M1', M3: 'PERIOD_M5', M4: 'PERIOD_M5', M5: 'PERIOD_M5', M6: 'PERIOD_M5',
  M10: 'PERIOD_M15', M12: 'PERIOD_M15', M15: 'PERIOD_M15', M20: 'PERIOD_M30', M30: 'PERIOD_M30',
  H1: 'PERIOD_H1', H2: 'PERIOD_H1', H3: 'PERIOD_H4', H4: 'PERIOD_H4', H6: 'PERIOD_H4', H8: 'PERIOD_H4', H12: 'PERIOD_H4',
  D1: 'PERIOD_D1', W1: 'PERIOD_W1', MN1: 'PERIOD_MN1',
};
...
void Activate(){
   ...
   string body="{"+qq+"key"+qq+":"+qq+InpApiKey+qq+","+qq+"account"+qq+":"+(string)AccountNumber()+","
      +qq+"magic"+qq+":"+(string)InpMagic+","+qq+"build"+qq+":"+qq+ONYX_BUILD+qq+","+qq+"creator"+qq+":"+qq+ONYX_CREATOR+qq+"}";
   char post[], res[]; string headers="Content-Type: application/json\\r\\n";
   int blen=StringToCharArray(body,post,0,WHOLE_ARRAY,CP_UTF8)-1; if(blen<0) blen=0; ArrayResize(post,blen);
   string resH; ResetLastError();
   int r=WebRequest("POST",ONYX_SITE+"/api/v1/activate",headers,5000,post,res,resH);
   if(r!=200){
      ...
      gAuth=false; gAuthMsg="${T('Permite la URL de Onyx en Opciones > Asesores expertos', 'Allow the Onyx URL in Options > Expert Advisors')}"; return; }
   string j=CharArrayToString(res,0,WHOLE_ARRAY,CP_UTF8);
   bool allowed=(StringFind(j,qq+"allowed"+qq+":true")>=0);
   if(allowed){ gAuth=true; gAuthLastOk=TimeCurrent(); gAuthMsg="${T('Activo', 'Active')}"; }
   else { gAuth=false; gAuthMsg="${T('Clave/plan no activo para esta cuenta', 'Key/plan not active for this account')}"; }
}

//====================== TIEMPO / VENTANAS ======================
int RefMin(){ MqlDateTime d; TimeToStruct(TimeCurrent(),d); return d.hour*60+d.min; }
int RefDow(){ MqlDateTime d; TimeToStruct(TimeCurrent(),d); return d.day_of_week; }
int DayId(){ MqlDateTime d; TimeToStruct(TimeCurrent(),d); return d.year*1000+d.day_of_year; }
bool InRange(int m,int f,int t){ return (f<=t)?(m>=f&&m<=t):(m>=f||m<=t); }
bool InWindow(){ int m=RefMin();
   if(StringLen(InpWindows)<3) return InRange(m, InpSignalFromH*60+InpSignalFromM, InpSignalToH*60+InpSignalToM);
   string parts[]; int n=StringSplit(InpWindows,(ushort)',',parts);
   for(int i=0;i<n;i++){ string ab[]; if(StringSplit(parts[i],(ushort)'-',ab)==2){ if(InRange(m,(int)StringToInteger(ab[0]),(int)StringToInteger(ab[1]))) return true; } }
   return false; }
bool DayOperable(){ int w=RefDow(); return ((InpTradeDays>>w)&1)==1; }
bool ForceClose(){ if(!InpUseDayClose) return false; return RefMin()>=InpForceCloseHourNY*60+InpForceCloseMinNY; }
bool FridayCut(){ if(!InpNoWeekend) return false; if(RefDow()!=5) return false; return RefMin()>=InpFridayHour*60; }
bool NewBar(){ datetime t=iTime(S,InpTF,0); if(t!=lastBar){ lastBar=t; return true; } return false; }

//====================== MERCADO / SIMBOLO ======================
double Atr(){ return iATR(S,InpTF,InpATRPeriod,1); }
double PipSize(){ return point*((gDigits==3||gDigits==5)?10.0:1.0); }
double TickVal(){ return MarketInfo(S,MODE_TICKVALUE); }
double TickSz(){ return MarketInfo(S,MODE_TICKSIZE); }
double Bid(){ return MarketInfo(S,MODE_BID); }
double Ask(){ return MarketInfo(S,MODE_ASK); }
string NormSym(string x){ StringToUpper(x); string o=""; for(int i=0;i<StringLen(x);i++){ ushort c=StringGetCharacter(x,i); if((c>='A'&&c<='Z')||(c>='0'&&c<='9')) o+=ShortToString(c); } return o; }
void Aliases(string want,string &out[]){ string w=NormSym(want); ArrayResize(out,1); out[0]=w;
   string groups[]={ "XAUUSD,GOLD,GOLDUSD", "XAGUSD,SILVER,SILVERUSD", "US30,DJ30,WS30,DOW,DJIA,US30USD,USA30", "NAS100,USTEC,NDX,US100,NAS,NASDAQ,USATECH", "SPX500,US500,SP500,SPX,USA500", "GER40,DE40,DAX,DAX40,GER30", "UK100,FTSE100,FTSE", "USOIL,WTI,CRUDE,OIL,USOUSD,XTIUSD", "BTCUSD,BITCOIN,BTC", "ETHUSD,ETHEREUM,ETH" };
   for(int g=0;g<ArraySize(groups);g++){ string parts[]; int k=StringSplit(groups[g],(ushort)',',parts); bool in=false; for(int p=0;p<k;p++) if(NormSym(parts[p])==w){ in=true; break; }
      if(in) for(int p2=0;p2<k;p2++){ string a=NormSym(parts[p2]); int m=ArraySize(out); ArrayResize(out,m+1); out[m]=a; } } }
string ResolveSymbol(string want){ if(want==""||want==NULL) return Symbol(); if(MarketInfo(want,MODE_BID)>0) return want;
   string al[]; Aliases(want,al); int tot=SymbolsTotal(false);
   for(int i=0;i<tot;i++){ string nm=SymbolName(i,false); string x=NormSym(nm); for(int a=0;a<ArraySize(al);a++) if(x==al[a]) return nm; }
   for(int i2=0;i2<tot;i2++){ string nm=SymbolName(i2,false); string x=NormSym(nm); for(int a2=0;a2<ArraySize(al);a2++) if(StringLen(al[a2])>=3 && StringFind(x,al[a2])==0) return nm; }
   return Symbol(); }

//====================== UNIDADES ======================
double UnitDist(int unit,double val,double base,double lots,double atr){
   if(val<=0) return 0;
   if(unit==0) return val*PipSize();
   if(unit==4) return val*atr;
   if(unit==2){ double px=Bid(); return px*val/100.0; }
   if(unit==1) return val*base;
   double ts=TickSz(), tv=TickVal(); if(ts<=0||tv<=0||lots<=0) return (base>0?val*base:val*atr); return val*ts/(lots*tv); }
double SLdist(double atr){ double d=UnitDist(InpSLUnit,InpSLValue,atr,0,atr); if(d<=0) d=(atr>0?atr:PipSize()*10); return d; }
double TPdist(double slDist,double lots,int unit,double val,double atr){ double d=UnitDist(unit,val,slDist,lots,atr); if(d<=0) d=slDist*2.0; return d; }
double TrailDist(double atr){ double d=UnitDist(InpTrailUnit,InpTrailValue,riskPx,gInitVol,atr); if(d<=0) d=atr; return d; }
double RiskMoney(){ double eq=AccountEquity(); if(InpSLUnit==3) return InpSLValue; if(InpRiskUnit==1) return InpRiskValue; return eq*InpRiskValue/100.0; }
double DailyCapMoney(){ if(InpDailyLossUnit==1) return InpDailyLossValue; return dayStartEq*InpDailyLossValue/100.0; }
double DailyTgtMoney(){ if(InpDailyProfitUnit==1) return InpDailyProfitValue; return dayStartEq*InpDailyProfitValue/100.0; }

//====================== SESGO / SEÑAL ======================
int TrendDir(){
   if(InpTrendMode==0){ double m=iMA(S,InpTrendTF,InpTrendMA,0,MODE_EMA,PRICE_CLOSE,0); double px=Bid(); return (px>m)?1:((px<m)?-1:0); }
   if(InpTrendMode==2){ int N=InpTrendDonchN; if(N<2)N=2; double hh=-100000000,ll=100000000; for(int i=1;i<=N;i++){ double h=iHigh(S,InpTrendTF,i),l=iLow(S,InpTrendTF,i); if(h>hh)hh=h; if(l<ll)ll=l; } double mid=(hh+ll)/2.0, px=Bid(); return (px>mid)?1:((px<mid)?-1:0); }
   int k=InpTrendSwing; if(k<1)k=1; double sh[2]; double sl[2]; int nh=0,nl=0;
   for(int i=k+1;i<=250-k-1 && (nh<2||nl<2);i++){ bool isH=true,isL=true;
      for(int j=1;j<=k;j++){ if(iHigh(S,InpTrendTF,i)<=iHigh(S,InpTrendTF,i-j)||iHigh(S,InpTrendTF,i)<=iHigh(S,InpTrendTF,i+j)) isH=false; if(iLow(S,InpTrendTF,i)>=iLow(S,InpTrendTF,i-j)||iLow(S,InpTrendTF,i)>=iLow(S,InpTrendTF,i+j)) isL=false; if(!isH&&!isL) break; }
      if(isH && nh<2){ sh[nh]=iHigh(S,InpTrendTF,i); nh++; } if(isL && nl<2){ sl[nl]=iLow(S,InpTrendTF,i); nl++; } }
   if(nh<2||nl<2) return 0; if(sh[0]>sh[1] && sl[0]>sl[1]) return 1; if(sh[0]<sh[1] && sl[0]<sl[1]) return -1; return 0; }
int EntrySignal(){
   if(InpEntry==1){ double f1=iMA(S,InpTF,InpMAfast,0,MODE_EMA,PRICE_CLOSE,1),f2=iMA(S,InpTF,InpMAfast,0,MODE_EMA,PRICE_CLOSE,2); double s1=iMA(S,InpTF,InpMAslow,0,MODE_EMA,PRICE_CLOSE,1),s2=iMA(S,InpTF,InpMAslow,0,MODE_EMA,PRICE_CLOSE,2); bool up=(f2<=s2 && f1>s1); bool dn=(f2>=s2 && f1<s1); return up?1:(dn?-1:0); }
   if(InpEntry==2){ double v1=iRSI(S,InpTF,InpRSIperiod,PRICE_CLOSE,1),v2=iRSI(S,InpTF,InpRSIperiod,PRICE_CLOSE,2); bool up=(v2<=InpRSIos && v1>InpRSIos); bool dn=(v2>=InpRSIob && v1<InpRSIob); return up?1:(dn?-1:0); }
   if(InpEntry==3){ int N=InpDonchN; if(N<2)N=2; double hh=-100000000,ll=100000000; for(int i=1;i<=N;i++){ double h=iHigh(S,InpTF,i),l=iLow(S,InpTF,i); if(h>hh)hh=h; if(l<ll)ll=l; } double c=iClose(S,InpTF,0); if(c>hh) return 1; if(c<ll) return -1; return 0; }
   if(InpEntry==4){ MqlDateTime d; TimeToStruct(TimeCurrent(),d); if(d.hour==InpEntryHour && d.min<5) return 1; return 0; }
   int k=InpMicroSwing; if(k<1)k=1; double hi=0,lo=0;
   for(int i=k+1;i<=110-k-1;i++){ bool isH=true; for(int j=1;j<=k;j++) if(iHigh(S,InpTF,i)<=iHigh(S,InpTF,i-j)||iHigh(S,InpTF,i)<=iHigh(S,InpTF,i+j)){ isH=false; break; } if(isH){ hi=iHigh(S,InpTF,i); break; } }
   for(int i2=k+1;i2<=110-k-1;i2++){ bool isL=true; for(int j2=1;j2<=k;j2++) if(iLow(S,InpTF,i2)>=iLow(S,InpTF,i2-j2)||iLow(S,InpTF,i2)>=iLow(S,InpTF,i2+j2)){ isL=false; break; } if(isL){ lo=iLow(S,InpTF,i2); break; } }
   double c1=iClose(S,InpTF,1); if(hi>0 && c1>hi) return 1; if(lo>0 && c1<lo) return -1; return 0; }

//====================== RIESGO / EJECUCION ======================
double CalcLots(double slDist){ if(slDist<=0) return 0; double riskMoney=RiskMoney(); double ts=TickSz(), tv=TickVal(); if(ts<=0||tv<=0) return 0;
   double lots=riskMoney/((slDist/ts)*tv); double step=MarketInfo(S,MODE_LOTSTEP), mn=MarketInfo(S,MODE_MINLOT), mx=MarketInfo(S,MODE_MAXLOT);
   if(step>0) lots=MathFloor(lots/step)*step; if(lots<mn) lots=mn; if(lots>mx) lots=mx; if(lots>InpMaxLots) lots=InpMaxLots; return NormalizeDouble(lots,2); }
double AdjVol(double v){ double step=MarketInfo(S,MODE_LOTSTEP),mn=MarketInfo(S,MODE_MINLOT); if(step>0) v=MathFloor(v/step)*step; if(v<mn) v=0; return NormalizeDouble(v,2); }
int CountMine(){ int n=0; for(int i=OrdersTotal()-1;i>=0;i--){ if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)){ if(OrderMagicNumber()==InpMagic && OrderSymbol()==S && OrderType()<=OP_SELL) n++; } } return n; }
void CloseMine(string why){ for(int i=OrdersTotal()-1;i>=0;i--){ if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)){ if(OrderMagicNumber()==InpMagic && OrderSymbol()==S && OrderType()<=OP_SELL){ double px=(OrderType()==OP_BUY)?Bid():Ask(); OrderClose(OrderTicket(),OrderLots(),NormalizeDouble(px,gDigits),30,clrNONE); } } } }

void OpenTrade(int dir){ double atr=Atr(); if(atr<=0) return; double slDist=SLdist(atr); if(slDist<minStop) slDist=minStop; if(atr>0 && slDist<0.2*atr) slDist=0.2*atr;
   double lots=CalcLots(slDist); if(lots<=0) return;
   double runDist=TPdist(slDist,lots,InpRunnerUnit,InpRunnerValue,atr); if(runDist<slDist) runDist=slDist*2.0;
   double entry=(dir>0)?Ask():Bid(); double sl=(dir>0)?entry-slDist:entry+slDist; double tp=(dir>0)?entry+runDist:entry-runDist;
   sl=NormalizeDouble(sl,gDigits); tp=NormalizeDouble(tp,gDigits); entry=NormalizeDouble(entry,gDigits);
   int t=OrderSend(S,(dir>0)?OP_BUY:OP_SELL,lots,entry,30,sl,tp,InpComment,InpMagic,0,(dir>0)?clrDodgerBlue:clrOrangeRed);
   if(t>0){ gTradesToday++; gTP1done=false; } }

bool SelectMine(){ for(int i=OrdersTotal()-1;i>=0;i--){ if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)){ if(OrderMagicNumber()==InpMagic && OrderSymbol()==S && OrderType()<=OP_SELL) return true; } } return false; }
void Manage(){ if(!SelectMine()){ gTicket=0; gTP1done=false; return; }
   double atr=Atr(); int tk=OrderTicket();
   if(tk!=gTicket){ gTicket=tk; entryPx=OrderOpenPrice(); gType=OrderType(); gInitVol=OrderLots(); openBar=lastBar;
      double sl0=OrderStopLoss(); riskPx=(sl0>0)?MathAbs(entryPx-sl0):SLdist(atr); if(riskPx<=0) riskPx=(atr>0?atr:PipSize()*10);
      gTP1dist=TPdist(riskPx,gInitVol,InpTP1Unit,InpTP1Value,atr); if(gTP1dist<=0) gTP1dist=riskPx; gTP1done=false; }
   double fav=(gType==OP_BUY)?Bid()-entryPx:entryPx-Ask();
   if(!gTP1done){
      if(InpTimeStopBars>0){ int held=(int)((lastBar-openBar)/PeriodSeconds(InpTF)); if(held>=InpTimeStopBars && fav<0.5*riskPx){ CloseMine("Time-stop"); return; } }
      if(fav>=gTP1dist && gTP1dist>0){ double cv=AdjVol(gInitVol*InpPartialPct/100.0);
         if(cv>0 && cv<OrderLots()){ double px=(gType==OP_BUY)?Bid():Ask(); OrderClose(tk,cv,NormalizeDouble(px,gDigits),30,clrNONE); }
         if(SelectMine()){ tk=OrderTicket(); double be=NormalizeDouble((gType==OP_BUY)?entryPx+InpBEOffsetR*riskPx:entryPx-InpBEOffsetR*riskPx,gDigits); OrderModify(tk,OrderOpenPrice(),be,OrderTakeProfit(),0,clrNONE); }
         gTP1done=true; }
   } else if(InpUseTrail){ double td=TrailDist(atr); if(td>0){ double csl=OrderStopLoss(),ctp=OrderTakeProfit();
      if(gType==OP_BUY){ double nsl=NormalizeDouble(Bid()-td,gDigits); if(nsl>csl && Bid()-nsl>=minStop) OrderModify(tk,OrderOpenPrice(),nsl,ctp,0,clrNONE); }
      else { double nsl=NormalizeDouble(Ask()+td,gDigits); if((csl==0||nsl<csl) && nsl-Ask()>=minStop) OrderModify(tk,OrderOpenPrice(),nsl,ctp,0,clrNONE); } } } }

//====================== OBJETIVO / DD / STATS ======================
double TargetPct(){ if(InpAccountMode==0) return InpTargetP1; if(InpAccountMode==1) return InpTargetP2; return 0; }
double ProfitPct(){ if(gInitBal<=0) return 0; return 100.0*(AccountEquity()-gInitBal)/gInitBal; }
double DDFloor(){ double L=InpAcctMaxDDPct/100.0; if(InpDDType==1 && gInitBal>0) return gInitBal*(1.0-L);
   if(InpDDType==2 && gInitBal>0){ double f=gPeakEq-L*gInitBal; return (f>gInitBal)?gInitBal:f; } return gPeakEq*(1.0-L); }
double DDPct(){ double ref=(InpDDType==1 && gInitBal>0)?gInitBal:gPeakEq; double eq=AccountEquity(); return (ref>0)?100.0*(ref-eq)/ref:0; }
void ComputeStats(){ stTrades=0; stWins=0; stNet=0; for(int i=0;i<OrdersHistoryTotal();i++){ if(OrderSelect(i,SELECT_BY_POS,MODE_HISTORY)){ if(OrderMagicNumber()!=InpMagic||OrderSymbol()!=S||OrderType()>OP_SELL) continue; double p=OrderProfit()+OrderSwap()+OrderCommission(); stTrades++; stNet+=p; if(p>=0) stWins++; } } stWR=stTrades>0?100.0*stWins/stTrades:0; }

//====================== NOTICIAS ======================
string QQ(){ return CharToString((uchar)34); }
int JImp(string s2){ if(StringFind(s2,"High")>=0) return 0; if(StringFind(s2,"Medium")>=0) return 1; if(StringFind(s2,"Low")>=0) return 2; return 3; }
string JVal(string rec,string key){ string q2=QQ(); string k=q2+key+q2+":"+q2; int p=StringFind(rec,k); if(p>=0){ p+=StringLen(k); int e=StringFind(rec,q2,p); if(e<0) return ""; return StringSubstr(rec,p,e-p); }
   k=q2+key+q2+":"; p=StringFind(rec,k); if(p<0) return ""; p+=StringLen(k); int e2=StringFind(rec,",",p); if(e2<0) e2=StringFind(rec,"}",p); if(e2<0) return ""; return StringSubstr(rec,p,e2-p); }
datetime ParseFF(string d){ if(StringLen(d)<16) return 0; MqlDateTime t; t.year=(int)StringToInteger(StringSubstr(d,0,4)); t.mon=(int)StringToInteger(StringSubstr(d,5,2)); t.day=(int)StringToInteger(StringSubstr(d,8,2)); t.hour=(int)StringToInteger(StringSubstr(d,11,2)); t.min=(int)StringToInteger(StringSubstr(d,14,2)); t.sec=0; datetime local=StructToTime(t);
   int off=0; if(StringLen(d)>=25){ int sign=(StringGetCharacter(d,19)=='-')?-1:1; int oh=(int)StringToInteger(StringSubstr(d,20,2)); int om=(int)StringToInteger(StringSubstr(d,23,2)); off=sign*(oh*3600+om*60); }
   datetime gmt=local-off; return gmt+InpServerGmt*3600; }
void FetchNews(){ nwLast=TimeCurrent(); if(!InpUseNews) return; ArrayResize(nwT,0); ArrayResize(nwImp,0); ArrayResize(nwCur,0);
   string url="https://nfs.faireconomy.media/ff_calendar_thisweek.json"; char post[]; char res[]; string rh; ResetLastError();
   int r=WebRequest("GET",url,"",5000,post,res,rh);
   if(r!=200){ nwFail=true; return; }
   nwFail=false; string body=CharArrayToString(res,0,WHOLE_ARRAY,CP_UTF8); StringReplace(body,"},{","}|{"); string recs[]; int n=StringSplit(body,(ushort)'|',recs);
   for(int i=0;i<n;i++){ string cur=JVal(recs[i],"country"); if(cur=="") cur=JVal(recs[i],"currency"); datetime tt=ParseFF(JVal(recs[i],"date")); if(tt<=0) continue; int ic=JImp(JVal(recs[i],"impact")); int m=ArraySize(nwT); ArrayResize(nwT,m+1); ArrayResize(nwImp,m+1); ArrayResize(nwCur,m+1); nwT[m]=tt; nwImp[m]=ic; nwCur[m]=cur; } }
bool InNews(){ if(!InpUseNews) return false; datetime now=TimeCurrent();
   for(int i=0;i<ArraySize(nwT);i++){ if(nwImp[i]>InpNewsImpact) continue; if(StringLen(InpNewsCur)>0 && StringFind(InpNewsCur,nwCur[i])<0) continue;
      if(now>=nwT[i]-InpNewsBefore*60 && now<=nwT[i]+InpNewsAfter*60) return true; } return false; }
...
//====================== INIT / TICK / MOTOR ======================
int OnInit(){ S=ResolveSymbol(InpSymbol);
   point=MarketInfo(S,MODE_POINT); gDigits=(int)MarketInfo(S,MODE_DIGITS);
   double a=MarketInfo(S,MODE_STOPLEVEL),f2=MarketInfo(S,MODE_FREEZELEVEL); minStop=MathMax(a,f2)*point;
   curDayId=DayId(); dayStartEq=AccountEquity(); gPeakEq=AccountEquity();
   gInitBal=(InpInitBalance>0)?InpInitBalance:AccountBalance();
   ComputeStats(); if(InpUseNews) FetchNews(); Activate(); EventSetTimer(2); Print(${q(s.name)},"${T(' iniciado en ', ' started on ')}",S); return INIT_SUCCEEDED; }
void OnDeinit(const int reason){ EventKillTimer(); ObjectsDeleteAll(0,PFX); }
void OnTimer(){ Engine(); Panel(); }
void OnTick(){ Engine(); Panel(); }
void DailyReset(){ int d=DayId(); if(d!=curDayId){ curDayId=d; gTradesToday=0; dayLocked=false; gDayGoal=false; dayStartEq=AccountEquity(); } }

void Engine(){ DailyReset(); bool nb=NewBar(); Manage();
   if(TimeCurrent()-gAuthLastTry>3600) Activate();
   double eq=AccountEquity(); if(eq>gPeakEq) gPeakEq=eq;
   if(TimeCurrent()%30==0) ComputeStats();
   if(InpUseNews && TimeCurrent()-nwLast>1800) FetchNews(); gInNews=InNews();
   if(ForceClose() && CountMine()>0) CloseMine("Cierre de sesion");
   if(FridayCut() && CountMine()>0) CloseMine("Fin de semana");
