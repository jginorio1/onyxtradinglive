...
void OnTimer()
{
   static int tick = 0;
   tick++;
   // La sincronizacion pesada solo cada N segundos; el panel late cada segundo.
   // Si la cuenta es MASTER de copy, sincroniza mas rapido (3s) para copiar antes.
   int every = (g_featMaster ? 3 : SYNC_SECS);
   if(tick % every == 0)
   {
      Sync();
      EnforceGuard();
