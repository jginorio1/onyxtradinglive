...
input bool   InpResyncAll     = false;   // Reenviar TODO el historial una vez (corrige datos viejos)
...
int OnInit()
  {
   ...
   if(InpResyncAll)  // reenviar todo el historial (corrige lotaje/direccion de operaciones viejas)
     { GlobalVariableDel(gv); lastSyncedClose=TimeCurrent()-(datetime)InpHistoryDays*86400; Print("Onyx: reenvio total de historial ACTIVADO."); }
...
string ClosedTradesJson(datetime fromT, datetime &maxClose, int &count)
  {
   ...
   // Miramos 10 dias hacia atras del punto de sincronizacion para capturar la
   // operacion de APERTURA de posiciones que se cerraron poco despues (evita lotaje 0).
   if(!HistorySelect(fromT-10*86400, TimeCurrent()+60)) return "[]";
   ...
   for(int i=0;i<total;i++)
     {
      ...
      if(entry==DEAL_ENTRY_IN)
      ...
      else if(entry==DEAL_ENTRY_OUT || entry==DEAL_ENTRY_OUT_BY)
        {
         ...
         // Respaldo: si no se vio la apertura, tomar lotaje y direccion del cierre.
         if(pvol[at]<=0) pvol[at]=HistoryDealGetDouble(tk,DEAL_VOLUME);
         if(pdir[at]==0) pdir[at]=(HistoryDealGetInteger(tk,DEAL_TYPE)==DEAL_TYPE_SELL)?1:-1;
        }
     }
