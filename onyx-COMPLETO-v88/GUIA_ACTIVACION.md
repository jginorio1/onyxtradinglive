...
## 55 · Metas de ganancia que se guardan de verdad (Fase 21)

Antes las metas semanal/mensual/anual se guardaban solo en el navegador y "se
borraban" al limpiar caché o cambiar de dispositivo. Ahora viven en el perfil.

1. **Corre `supabase/goals.sql`** (o el bloque equivalente ya incluido en
   `_SETUP_ALL.sql`). Añade `goal_week`, `goal_month`, `goal_year` a `profiles`.
   Idempotente.
2. Nada más. El trader fija sus metas en **Logros y metas → Fijar metas** y el
   servidor confirma lo guardado; si algo falla, ahora avisa en rojo en vez de
   aparentar que guardó. **Si sigues viendo que se borran, es que no corriste el SQL.**

---

## 56 · Ganancias parciales + Runner + "Mercado cerrado" (Fase 22)

Tres cosas que van juntas. La estadística de parciales sale del historial real del
bróker, así que funciona lo cierre quien lo cierre (tu bot, el Guardian o tú).

### 56.1 · SQL (una vez)
- **Corre `supabase/partials.sql`** (o el bloque ya incluido en `_SETUP_ALL.sql`).
  Añade `position_id`, `exit_reason` y `closed_volume` a `trades`. El sync es
  **tolerante**: si aún no lo corres, no se rompe nada (solo no verás el desglose
  de parciales y el dashboard mostrará "actualiza tu EA").

### 56.2 · Reinstala el EA (obligatorio para el desglose y el reloj)
Los EAs se actualizaron. **Vuelve a descargar e instalar** el que uses:
- **MT5** (`OnyxConnect_MT5.mq5`): reporta la posición (agrupa TP1/TP2/runner) y el
  **motivo de salida real** (TP/SL/stop out). Preciso.
- **MT4** (`OnyxConnect_MT4.mq4`): mejor esfuerzo (agrupa por la cadena de
  comentarios; deduce el motivo comparando el cierre con TP/SL).
- **cTrader** (`ctrader/OnyxConnect.cs`): usa el id de cada cierre para no pisar los
  parciales y agrupa por posición.
- **Match-Trader**: no tiene EA; su puente/API debe enviar los mismos campos
  (`positionId`, `exitReason`, `closedVolume`) a `/api/v1/sync` y todo aparece igual.
> Como todo cambio de EA, **pruébalo primero en demo**.

### 56.3 · Qué ves en el dashboard
Tarjeta **"Salidas · Full TP vs parciales"**: Full TP, Cierre parcial, Ganancia
parcial banqueada y Aporte del runner, más el desglose por **Motivo de salida**
(TP / trailing / manual / SL). Los porcentajes se calculan sobre las operaciones
**con dato de salida** (las que trae el EA nuevo), no sobre el histórico viejo.
Nota honesta: en MT4/MT5 un **trailing** es un stop movido, así que el bróker lo
reporta como **SL** — el trailing figura dentro de "Stop (SL)". El "aporte del
runner" sí lo captura bien por el último cierre de la posición.

### 56.4 · Runner en el Guardian (Take profits parciales)
El 4º nivel dejó de ser "TP4" y ahora es el **Runner**: no lleva % propio, cierra
"el resto" (100 − suma de los TP) o **se queda corriendo con el trailing** si
activas "Dejar correr". Los % de los TP son **del tamaño original** de la posición
y deben sumar ≤ 100; el resto es el runner. Sin SQL nuevo (la config va en el JSON
del gestor). Requiere el plan con Guardian.

### 56.5 · "Mercado cerrado · abre en HH:MM:SS"
- **En el panel del EA**: MT5 y cTrader lo muestran al segundo con las sesiones
  reales del bróker; MT4 lo estima por el horario semanal del Forex (aprox.).
- **En el dashboard de Onyx**: un reloj de mercado con cuenta atrás para todas las
  plataformas (incluida Match-Trader, que no tiene panel propio). Sin configuración.

---

## 57 · Blog con imágenes (alt por IA) y gráficas (Fase 23)

El blog ahora admite imagen de portada e imágenes en el cuerpo con **texto
alternativo (alt) autocompletado por la IA**, y **gráficas** que la IA rellena con
datos de ejemplo.

1. **Corre `supabase/blog_images.sql`** (o el bloque en `_SETUP_ALL.sql`). Añade
   `cover_alt_es` y `cover_alt_en` a `blog_posts`.
2. **Bucket de Storage**: reutiliza el bucket público **`academy`** (el mismo de la
   academia). Si ya lo tienes, no hay que crear nada; si no, créalo público.
3. En **Admin → Blog**: la portada tiene botón "Subir imagen" + alt ES/EN con
   "✨ Generar alt con IA"; en el cuerpo, "Insertar imagen" (sube + alt automático)
   y "Insertar gráfica" (bloque `:::chart` editable). Las gráficas usan siempre
   **datos de ejemplo** (nunca señales ni promesas) y se dibujan con Chart.js.
   El "Repaso de Onyx AI" del plan del trader ahora escribe con estructura y emojis,
   sin asteriscos crudos (sin acción de tu parte).

> Nota: el contador de números del landing ahora usa formato compacto automático
> (17.4K, 1.2M, 100M…), así que el "+" nunca se corta por muchos que crezcan.
