...
function statColor(s: any) { return s.ok && !s.warn ? 'var(--green)' : s.warn ? 'var(--amber)' : s.ok ? 'var(--green)' : 'var(--red)'; }
    ...
    <div>
      <div className="row between" style={{ flexWrap: 'wrap', gap: 8 }}>
        ...
        <div className="row" style={{ gap: 8 }}>
          <span className="pill" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--soft-green)', background: 'rgba(52,226,160,.15)' }}><span className="livedot" />{t.d_liveBadge}</span>
          ...
          {d.errors.map((e: any, i: number) => (
            <div key={i} className="card" style={{ padding: '10px 12px' }}>
              <div style={{ fontSize: 13 }}>
                ...
                <b>{e.source}</b> <span className="muted" style={{ fontSize: 11 }}>{fmtDateTime(e.created_at, lang)}</span>
              </div>
