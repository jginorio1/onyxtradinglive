...
export default function LandingConstructor() {
  ...
  const faqs = [
    [L('¿En qué se diferencia "Crea tu bot" de Bot Lab?', 'How is "Build a bot" different from Bot Lab?'), L('Aquí, en "Crea tu bot", eres tú quien ARMA el robot paso a paso, sin código, a la medida de tu estrategia, y lo descargas listo para instalar. Bot Lab es el MARKETPLACE: ahí COMPRAS robots ya hechos por traders verificados (o vendes los tuyos), sin construir nada — para quien quiere resultados rápidos. Y se conectan: cuando un robot que creas aquí acumula historial real, lo puedes publicar en Bot Lab y cobrar a otros por usarlo.', 'Here, in "Build a bot", YOU assemble the robot step by step, no code, tailored to your strategy, and download it ready to install. Bot Lab is the MARKETPLACE: there you BUY ready-made robots from verified traders (or sell yours), no building — for those who want fast results. And they connect: once a robot you build here has a real track record, you can list it on Bot Lab and charge others to use it.')],
