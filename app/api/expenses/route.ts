...
// ¿El plan del usuario incluye "Balance real"? (capacidad expenses)
...
function monthRange(month?: string | null) {
  ...
  const start = `${m}-01`;
  const endDate = new Date(y, mo, 0); // último día del mes
  const end = `${m}-${String(endDate.getDate()).padStart(2, '0')}`;
  return { m, start, end };
}

// GET · resumen del mes: bruto (trading) − gastos = neto, + por categoría + items.
export async function GET(req: Request) {
  try {
    ...
    const { m, start, end } = monthRange(new URL(req.url).searchParams.get('month'));

    // Bruto de trading del mes (suma de net_profit de las operaciones cerradas).
    let gross = 0;
    ...
    // Gastos: los del mes + los recurrentes con fecha de inicio <= fin de mes.
    ...
    const totalExp = items.reduce((s: number, e: any) => s + (Number(e.amount) || 0), 0);
    const byCategory: Record<string, number> = {};
    for (const e of items) byCategory[e.category] = (byCategory[e.category] || 0) + (Number(e.amount) || 0);

    return NextResponse.json({
      locked: false, month: m,
      gross: Math.round(gross * 100) / 100,
      expenses: Math.round(totalExp * 100) / 100,
      net: Math.round((gross - totalExp) * 100) / 100,
      byCategory, items,
    });
  } catch (e: any) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!amount || amount <= 0) return NextResponse.json({ error: 'monto' }, { status: 400 });
    const row = {
      user_id: user.id,
    ...
    const { error } = await supabaseAdmin.from('expenses').insert(row);
...
// DELETE · borrar gasto (solo el propio)
