...
const BUCKET = 'sales-cv';
const MAX = 5 * 1024 * 1024; // 5 MB

// POST público · sube el currículum (PDF) del candidato a un bucket PRIVADO y
// devuelve la RUTA interna (el admin la abre con un enlace firmado). No expone URL pública.
export async function POST(req: Request) {
  try {
    ...
    if (up.error) return NextResponse.json({ error: up.error.message, hint: 'Crea el bucket privado "sales-cv" (corre sales_v4.sql).' }, { status: 500 });
