...
// capacidad "coach". Analiza los últimos ~90 días de operaciones cerradas.
export async function GET(req: Request) {
  try {
    ...
    const { data: trades } = await supabaseAdmin.from('trades').select('net_profit,symbol,close_time')
      .in('account_id', ids).gte('close_time', since).order('close_time', { ascending: true }).limit(50000);
    ...
    const summary: CoachSummary = {
      net: Math.round(net * 100) / 100, trades: rows.length,
      winRate: wins + losses ? Math.round((wins / (wins + losses)) * 100) : 0,
      pf: grossLoss ? Math.round((grossWin / grossLoss) * 100) / 100 : 0,
      avgWin: wins ? Math.round((grossWin / wins) * 100) / 100 : 0,
      avgLoss: losses ? Math.round((grossLoss / losses) * 100) / 100 : 0,
      maxLossStreak: maxStreak, biggestLoss: Math.round(biggestLoss * 100) / 100,
