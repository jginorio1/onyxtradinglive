...
export async function POST(req: Request) {
  try {
    ...
    const { data: prof } = await supabaseAdmin.from('profiles').select('stripe_subscription_id,extra_accounts,plan').eq('id', user.id).maybeSingle();
    // Los add-ons son un extra de los planes de pago: en Free no se venden cuentas sueltas.
    if ((prof?.plan || 'free') === 'free') return NextResponse.json({ error: 'Upgrade to a paid plan to add extra accounts.', code: 'free_no_addon' }, { status: 400 });
    ...
    const { data: plan } = await supabaseAdmin.from('plans').select('max_accounts').eq('id', prof?.plan || 'free').maybeSingle();
