...
export async function POST(req: Request) {
  ...
  if (a === 'folder_create') { try { const f = await createFolder({ userId: user.id, name: String(b.name || ''), color: b.color }); logAdmin(user.email || '', 'factory_folder_create', f?.id || '', { name: f?.name }).catch(() => {}); return NextResponse.json({ ok: true, folder: f }); } catch (e: any) { return NextResponse.json({ error: e?.message || 'error' }, { status: 400 }); } }
  ...
  if (a === 'unlink_demo') { try { const r = await unlinkDemo(String(b.botId || '')); await logAdmin(user.email || '', 'factory_unlink_demo', String(b.botId || ''), {}); return NextResponse.json(r); } catch (e: any) { return NextResponse.json({ error: e?.message || 'error' }, { status: 400 }); } }
