...
import { mergeConfig, sanitize, partialsTotal, DEFAULT_CONFIG } from '@/lib/manager';
...
export async function GET() {
  try {
    ...
    const { data: accounts } = await supabaseAdmin
      .from('trading_accounts').select('id,login,nickname,broker,acc_type,last_sync_at,ea_version')
    ...
    return NextResponse.json({ accounts: rows, templates: templates || [], events: events || [], caps, defaults: DEFAULT_CONFIG });
  } catch (e: any) {
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, version: Number(prev?.version || 0) + 1 });
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || 'error' }, { status: 500 });
  }
}
