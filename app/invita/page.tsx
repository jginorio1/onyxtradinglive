...
export default function Invita() {
  ...
  useEffect(() => {
    fetch('/api/referral/info').then((r) => r.json()).then(setD).catch(() => setD({ referrerCredit: 10, friendCredit: 10, holdDays: 21, bridge: 5 }));
  ...
  return (
    <div className="wrap" style={{ padding: '52px 22px 60px' }}>
      ...
      <p className="muted" style={{ textAlign: 'center', fontSize: 13, maxWidth: 620, margin: '0 auto 40px' }}>{fill(t.creditNote, { d: d?.holdDays ?? 21 })}</p>

      <h2 style={{ textAlign: 'center', marginBottom: 20 }}>{t.how}</h2>
      <div className="grid g3" style={{ marginBottom: 40 }}>
        {[['1', t.s1t, t.s1d], ['2', t.s2t, t.s2d], ['3', t.s3t, t.s3d]].map(([n, ti, de]) => (
