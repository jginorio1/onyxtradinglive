  ...
  return (
    <div className="wrap" style={{ padding: '64px 22px 80px', maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
      ...
      <p className="muted" style={{ margin: '14px auto 0', fontSize: 17 }}>
        {L('Las academias son privadas. Solo puedes unirte con el código, el enlace o el código QR que te comparta tu mentor.',
      ...
      <div className="card" style={{ marginTop: 26, textAlign: 'left' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}><OnyxIcon emoji="🔑" size={16} /><b>{L('¿Tienes un código?', 'Have a code?')}</b></div>
        <p className="muted" style={{ fontSize: 14 }}>{L('Entra a tu cuenta y pégalo en Dashboard → Onyx Academy → «Unirme a una academia».', 'Sign in and paste it in Dashboard → Onyx Academy → “Join an academy”.')}</p>
