      ...
      {open && (
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))', gap: 10 }}>
            ...
            <div style={box}><div style={lbl}>{L('Frescura máx (min)', 'Max age (min)')}</div><input type="number" min={60} max={4320} value={cfg.maxAgeMin} onChange={(e) => upd('maxAgeMin', Math.min(4320, Math.max(0, parseInt(e.target.value, 10) || 0)))} onBlur={() => upd('maxAgeMin', Math.max(720, cfg.maxAgeMin || 720))} style={{ margin: 0, width: '100%' }} /></div>
