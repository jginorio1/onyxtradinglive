      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="row between" style={{ marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
          ...
          <span className="row" style={{ gap: 8, flexWrap: 'wrap' }}>
            ...
            {recent
              ? <span className="pill" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--soft-green)', background: 'rgba(52,226,160,.15)' }}><span className="livedot" />{t.autoOn}</span>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 10 }}>
          {tile(t.last, backup.last_at ? fmtDateTime(backup.last_at, lang) : t.never, undefined, recent)}
        ...
        {shown.map((c: any, i: number) => (
          <div key={i} style={{ borderTop: '1px solid var(--line)', padding: '10px 0' }}>
            <div className="row between" style={{ gap: 8, flexWrap: 'wrap', fontSize: 13 }}>
              <span className="row" style={{ gap: 8, flex: 1, minWidth: 150 }}>{i === 0 && recent ? <span className="livedot" /> : <span style={{ width: 7 }} />}{fmtDateTime(c.at, lang)}</span>
