...
const SalesAdmin = dynamic(() => import('./SalesAdmin'), { ssr: false, loading: tabLoad });
const Retention = dynamic(() => import('./Retention'), { ssr: false, loading: tabLoad });
const TrackrecordSettings = dynamic(() => import('./TrackrecordSettings'), { ssr: false, loading: tabLoad });
...
const PayoutsHub = dynamic(() => import('./PayoutsHub'), { ssr: false, loading: tabLoad });
...
const Payroll = dynamic(() => import('./Payroll'), { ssr: false, loading: tabLoad });
const CareersAdmin = dynamic(() => import('./CareersAdmin'), { ssr: false, loading: tabLoad });
const CopyTradersAdmin = dynamic(() => import('./CopyTradersAdmin'), { ssr: false, loading: tabLoad });
...
type Tab = 'resumen' | 'facturacion' | 'ingresos' | 'finanzas' | 'academy' | 'usuarios' | 'correos' | 'campanas' | 'blog' | 'seo' | 'planes' | 'landing' | 'landingnew' | 'equipo' | 'nomina' | 'carreras' | 'embajadores' | 'ventas' | 'retencion' | 'pruebas' | 'firms' | 'catalogos' | 'modulos' | 'soporte' | 'chat' | 'kb' | 'diag' | 'recursos' | 'backups' | 'audit' | 'optim' | 'notif' | 'guias' | 'copytraders' | 'botlab' | 'factory' | 'pagos' | 'antifraude' | 'trackrecord' | 'ajustes';
...
export default function AdminClient({ meEmail, role, perms = {}, accounts, trades, hasPin = false, idleMin = 20 }: { meEmail: string; role: string; perms?: Record<string, string>; accounts: number; trades: number; hasPin?: boolean; idleMin?: number }) {
  ...
  const NAV_GROUPS: { g: string; items: [Tab, string, string][] }[] = [
    { g: t.g_op, items: [['resumen', '📊', t.nav_resumen], ['facturacion', '💳', lang === 'en' ? 'Billing' : 'Facturación'], ['usuarios', '👥', t.nav_usuarios], ['correos', '✉️', t.nav_correos], ['soporte', '🎫', t.nav_soporte], ['chat', '💬', lang === 'en' ? 'Team chat' : 'Chat equipo'], ['equipo', '🛡️', t.nav_equipo], ['nomina', '💵', lang === 'en' ? 'Payroll' : 'Nómina'], ['carreras', '💼', lang === 'en' ? 'Careers' : 'Carreras']] },
    ...
    { g: t.g_growth, items: [['campanas', '📣', lang === 'en' ? 'Campaigns' : 'Campañas'], ['blog', '📝', 'Blog'], ['seo', '🔎', 'SEO'], ['copytraders', '🏆', 'Onyx Copy'], ['trackrecord', '📈', lang === 'en' ? 'Public trackrecord' : 'Trackrecord público'], ['embajadores', '🎁', t.nav_embajadores], ['ventas', '🧑‍💼', lang === 'en' ? 'Sales team' : 'Red de ventas'], ['pagos', '💸', lang === 'en' ? 'Payouts' : 'Pagos y retiros'], ['antifraude', '🛡️', lang === 'en' ? 'Payments & chargebacks' : 'Pagos & chargebacks'], ['retencion', '🛟', t.nav_retencion]] },
      ...
      <div className="wrap-wide" style={{ padding: '22px 0' }}>
        <div className="adminlayout">
          ...
          <div style={{ minWidth: 0 }}>
            ...
            {tab === 'nomina' && <Payroll canManage={role === 'owner' || perms.equipo === 'manage'} />}
            ...
            {tab === 'trackrecord' && <TrackrecordSettings />}
            ...
            {tab === 'factory' && <Factory canManage={role === 'owner' || perms.modulos === 'manage'} />}
