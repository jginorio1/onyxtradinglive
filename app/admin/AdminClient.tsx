...
type Tab = 'resumen' | 'facturacion' | 'ingresos' | 'finanzas' | 'academy' | 'usuarios' | 'correos' | 'campanas' | 'blog' | 'seo' | 'planes' | 'landing' | 'landingnew' | 'equipo' | 'nomina' | 'embajadores' | 'ventas' | 'retencion' | 'pruebas' | 'firms' | 'catalogos' | 'modulos' | 'soporte' | 'chat' | 'kb' | 'diag' | 'recursos' | 'backups' | 'audit' | 'optim' | 'notif' | 'guias' | 'copytraders' | 'botlab' | 'factory' | 'pagos' | 'antifraude' | 'trackrecord' | 'ajustes';
...
export default function AdminClient({ meEmail, role, perms = {}, accounts, trades, hasPin = false, idleMin = 20 }: { meEmail: string; role: string; perms?: Record<string, string>; accounts: number; trades: number; hasPin?: boolean; idleMin?: number }) {
  ...
  const areaOf: Record<string, string> = { resumen: 'resumen', facturacion: 'planes', ingresos: 'planes', finanzas: 'finanzas', academy: 'academy', usuarios: 'usuarios', correos: 'usuarios', campanas: 'campanas', planes: 'planes', landing: 'planes', equipo: 'equipo', nomina: 'equipo', embajadores: 'embajadores', ventas: 'embajadores', retencion: 'retencion', pruebas: 'diag', firms: 'firms', catalogos: 'catalogos', modulos: 'modulos', botlab: 'modulos', blog: 'modulos', soporte: 'soporte', chat: 'chat', kb: 'soporte', diag: 'diag', recursos: 'diag', backups: 'ajustes', audit: 'ajustes', optim: 'ajustes', ajustes: 'ajustes', seo: 'ajustes', pagos: 'embajadores', antifraude: 'planes' };
  ...
  const NAV_GROUPS: { g: string; items: [Tab, string, string][] }[] = [
    { g: t.g_op, items: [['resumen', '📊', t.nav_resumen], ['facturacion', '💳', lang === 'en' ? 'Billing' : 'Facturación'], ['usuarios', '👥', t.nav_usuarios], ['correos', '✉️', t.nav_correos], ['soporte', '🎫', t.nav_soporte], ['chat', '💬', lang === 'en' ? 'Team chat' : 'Chat equipo'], ['equipo', '🛡️', t.nav_equipo], ['nomina', '💵', lang === 'en' ? 'Payroll' : 'Nómina']] },
