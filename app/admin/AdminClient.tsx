...
type Tab = 'resumen' | 'facturacion' | 'ingresos' | 'finanzas' | 'academy' | 'usuarios' | 'correos' | 'campanas' | 'blog' | 'seo' | 'planes' | 'landing' | 'landingnew' | 'equipo' | 'embajadores' | 'retencion' | 'pruebas' | 'firms' | 'catalogos' | 'modulos' | 'soporte' | 'chat' | 'kb' | 'diag' | 'recursos' | 'backups' | 'audit' | 'optim' | 'notif' | 'guias' | 'copytraders' | 'botlab' | 'factory' | 'pagos' | 'antifraude' | 'trackrecord' | 'ajustes';
...
export default function AdminClient({ meEmail, role, perms = {}, accounts, trades, hasPin = false, idleMin = 20 }: { meEmail: string; role: string; perms?: Record<string, string>; accounts: number; trades: number; hasPin?: boolean; idleMin?: number }) {
  ...
  const NAV_GROUPS: { g: string; items: [Tab, string, string][] }[] = [
    ...
    { g: t.g_growth, items: [['campanas', '📣', lang === 'en' ? 'Campaigns' : 'Campañas'], ['blog', '📝', 'Blog'], ['seo', '🔎', 'SEO'], ['copytraders', '🏆', 'Onyx Copy'], ['trackrecord', '📈', lang === 'en' ? 'Public trackrecord' : 'Trackrecord público'], ['embajadores', '🎁', t.nav_embajadores], ['pagos', '💸', lang === 'en' ? 'Payouts' : 'Pagos y retiros'], ['antifraude', '🛡️', lang === 'en' ? 'Payments & chargebacks' : 'Pagos & chargebacks'], ['retencion', '🛟', t.nav_retencion]] },
