...
type Tab = 'resumen' | 'usuarios' | 'planes' | 'equipo' | 'embajadores' | 'retencion' | 'modulos' | 'ajustes';
...
export default function AdminClient({ meEmail, role, accounts, trades }: { meEmail: string; role: string; accounts: number; trades: number }) {
  ...
  const NAV: [Tab, string][] = [['resumen', '📊 Resumen'], ['usuarios', '👥 Usuarios'], ['planes', '💳 Planes'], ['equipo', '🛡️ Equipo'], ['embajadores', '🎁 Embajadores'], ['retencion', '🛟 Retención'], ['modulos', '🧩 Módulos'], ['ajustes', '⚙️ Ajustes']];

  return (
    <>
      <div className="topbar"><div className="wrap-wide">
        <Link className="logo" href="/dashboard"><img src="/onyx-symbol.png" alt="Onyx" style={{ width: 28, height: 28, objectFit: 'contain' }} /> Onyx · Súper-Admin</Link>
        <div className="row"><span className="pill" style={{ color: roleColor(role) }}>{role}</span><span className="muted" style={{ fontSize: 13 }}>{meEmail}</span><Link className="btn btn-ghost" href="/dashboard">Mi cuenta</Link></div>
      </div></div>
