      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <label style={{ ...btn('var(--brand)'), cursor: 'pointer' }}>{bars ? `${bars.length} barras · ${barsName.slice(0, 16)}` : (es ? 'Subir barras (CSV)' : 'Upload bars (CSV)')}
            <input type="file" accept=".csv,.txt,.tsv" style={{ display: 'none' }} onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; setBarsName(f.name); const b = parseBars(await f.text()); if (b.length < 100) toastErr(es ? 'Pocas barras leídas.' : 'Too few bars.'); setBars(b); }} />
          </label>
