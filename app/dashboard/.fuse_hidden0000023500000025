...
export default function Nudge({
  ...
  if (histDays > 0 && hiddenTrades > 0) {
    ...
    if (target) list.push({
      id: 'hidden_history', color: 'var(--brand)',
  ...
  if (!canJournal && totalTrades >= 100) {
    list.push({
      id: 'milestone_100', color: 'var(--gold)',
  ...
  list.push({
    id: 'ambassador', color: 'var(--brand2)',
