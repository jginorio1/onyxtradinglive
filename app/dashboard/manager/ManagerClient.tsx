...
function StateLabel({ on, on1, off1 }: { on: boolean; on1: string; off1: string }) {
  return <span style={{ fontSize: 11, fontWeight: 600, color: on ? 'var(--green)' : 'var(--mut)', minWidth: 52 }}>{on ? on1 : off1}</span>;
}

// Punto de estado: verde encendido, gris apagado.
function Dot({ on }: { on: boolean }) {
  return <span style={{ width: 9, height: 9, borderRadius: '50%', flex: 'none', background: on ? 'var(--green)' : 'var(--line)' }} />;
}
  ...
  return (
    <div className="row between" style={{ marginBottom: on ? 12 : 0, gap: 10, alignItems: 'center' }}>
      <div className="row" style={{ gap: 10, alignItems: 'center', flex: 1, minWidth: 0 }}>
        <Dot on={on} />
        <span style={{ display: 'inline-flex', color: 'var(--brand)' }}>{typeof icon === 'string' ? <OnyxIcon emoji={icon} size={20} /> : icon}</span>
