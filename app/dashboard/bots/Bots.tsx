    ...
    return (
      <div style={{
        ...
        <div className="row" style={{ gap: 6, marginTop: 'auto', flexWrap: 'wrap' }}>
          ...
          <Link href={`/dashboard/bot-lab?new=1&name=${encodeURIComponent(b.name || '')}&platform=${encodeURIComponent(b.platform || 'mt5')}`} title={L('Publícalo en Onyx Bot Lab y cobra a otros traders', 'List it on Onyx Bot Lab and charge other traders')} className="btn btn-ghost" style={{ fontSize: 11.5, padding: '5px 10px', color: 'var(--gold,#ffd45e)', borderColor: 'color-mix(in srgb,var(--gold,#ffd45e) 45%,transparent)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}><OnyxIcon emoji="💰" size={14} glow={false} /> {L('Vender', 'Sell')}</Link>
