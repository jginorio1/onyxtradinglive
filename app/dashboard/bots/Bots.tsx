    ...
    return (
      <div style={{
        ...
        <div className="row" style={{ gap: 6, marginTop: 'auto', flexWrap: 'wrap' }}>
          ...
          <Link href={`/dashboard/bot-lab?new=1&name=${encodeURIComponent(b.name || '')}&platform=${encodeURIComponent(b.platform || 'mt5')}&magic=${encodeURIComponent(b.magic ?? '')}&account=${encodeURIComponent(b.account_id ?? '')}`} title={L('Publícalo en Onyx Bot Lab con su track record real', 'List it on Onyx Bot Lab with its real track record')} className="btn btn-ghost" style={{ fontSize: 11.5, padding: '5px 10px', color: 'var(--gold,#ffd45e)', borderColor: 'color-mix(in srgb,var(--gold,#ffd45e) 45%,transparent)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5 }}><OnyxIcon emoji="💰" size={14} glow={false} /> {L('Vender', 'Sell')}</Link>
        ...
        <>
          ...
          {caps.portfolioLab && suggestions && (
            <div className="card" style={{ padding: '16px 16px 18px' }}>
              ...
              {!suggestions.distinct && <div style={{ fontSize: 12.5, color: 'var(--mut)', background: 'rgba(255,255,255,.04)', border: '1px solid var(--line)', borderRadius: 10, padding: '9px 12px', marginBottom: 12 }}><OnyxIcon emoji="ℹ️" size={12} glow={false} /> {L('Las opciones Defensivo, Equilibrado y Agresivo se separan cuando hay varios robots con operaciones. Aquí te mostramos la mejor combinación posible.', 'The Defensive, Balanced and Aggressive options split apart once several robots have trades. Here we show you the best combination possible.')}</div>}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 12 }}>
                {(suggestions.distinct ? [
                  ...
                  { key: 'best', icon: '✨', title: L('Mejor combinación disponible', 'Best available combo'), sub: L('A partir de tus robots con operaciones', 'Based on your robots with trades'), p: suggestions.best, hi: true },
                ]).map((s) => (
