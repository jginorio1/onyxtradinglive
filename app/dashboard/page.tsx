...
export default async function Dashboard() {
  ...
  const { data: accounts } = await sb.from('trading_accounts').select('id,login,broker,platform,balance,currency').eq('user_id', user.id);
  const accIds = (accounts || []).map((a: any) => a.id);

  let trades: any[] = [];
  if (accIds.length) {
    const { data } = await sb.from('trades')
      .select('symbol,side,volume,open_time,close_time,net_profit')
