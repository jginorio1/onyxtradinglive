...
export default async function BotLabLanding() {
  ...
  const tiers = [
    ...
    { lvl: es ? 'Nivel 3' : 'Tier 3', name: es ? 'Instalación asistida' : 'Assisted install', price: '$' + s.service_install_price, unit: es ? '/sesión' : '/session', desc: es ? 'Un experto instala y configura contigo.' : 'An expert sets it up with you live.', href: '#svc-install', cta: es ? 'Agendar' : 'Book', hot: false },
    { lvl: es ? 'Nivel 4' : 'Tier 4', name: es ? 'Automatiza tu estrategia' : 'Automate your strategy', price: '$' + s.service_automate_from.toLocaleString('en-US'), unit: '+', desc: es ? 'Convertimos tu estrategia en un robot a medida.' : 'We turn your strategy into a bespoke robot.', href: '#svc-automate', cta: es ? 'Solicitar' : 'Request', hot: true },
    { lvl: es ? 'Nivel 5' : 'Tier 5', name: es ? 'Elite / privado' : 'Elite / private', price: '$' + s.service_elite_from.toLocaleString('en-US'), unit: '+', desc: es ? 'Desarrollo privado, VPS y monitoreo con retainer.' : 'Private dev, VPS and monitoring with retainer.', href: '#svc-elite', cta: es ? 'Hablar' : 'Talk', hot: false },
  ];
